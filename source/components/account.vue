...
<template>
    <div>
        <secondary-header
            stringkey="menu_myaccount_title"
        ...
        <div class="account-container">
            <div class="account-info-container">
                <hint
                    ...
                    :text="expiryWarning"
                    iconName="error"
                    :type="state === 'expiring' ? 'warning' : 'error'"
                />
                <div class="account-info">
                    ...
                    <div class="account-info-label">
                        {{ localize('myaccount_account_status_label') }}
                    </div>
                    <div class="account-info-value">
                        {{
                            localize(
                                `myaccount_account_status_${this.getAccountStatus()}_text`
                            )
                        }}
                    </div>
                </div>
                <div class="account-info">
                    <img
                        v-svg-inline
                        class="icon chevron"
                        :src="`/images/icons/expiry.svg`"
                        viewbox="0 0 24 24"
                        width="24"
                        height="24"
                    />
                    <div class="account-info-label" id="account_expiry">
                        {{
                            localize(
                                `myaccount_account_${this.getExpiryType()}_label`
                            )
                        }}
                    </div>
                    <div class="account-info-value">
    ...
    computed: {
        ...
        expiryWarning() {
            ...
            if (this.state === 'expiring') {
                let expiringWhat = this.isTrialUser()
                    ? 'free_trial'
                    : 'subscription';
                let remainingTime =
                    ...
                    Date.now() / 1000;
                if (remainingTime <= 0) {
                    warning = this.localize(
                        `promo_bar_${expiringWhat}_expiring_soon_expired_text`
                    );
                } else if (remainingTime < 60 * 60) {
                    warning = this.localize(
                        `promo_bar_${expiringWhat}_expiring_soon_less_than_one_hour_text`
                    );
                } else if (remainingTime < 24 * 60 * 60) {
                    warning = this.localize(
                        `promo_bar_${expiringWhat}_expiring_soon_hours_text`
                    ).replace(
                        '[hours]',
                    ...
                    warning = this.localize(
                        `promo_bar_${expiringWhat}_expiring_soon_days_text`
                    ).replace(
                        '[days]',
                ...
                if (this.isTrialUser()) {
                    warning = this.localize(
                        'promo_bar_free_trial_expiring_soon_expired_text'
                    );
                } else if (!this.isPaymentMethodIAP()) {
                    warning = this.localize(
                        'promo_bar_subscription_expiring_soon_expired_text'
                    );
                }
            }
    ...
    methods: {
        ...
        getExpiryType: function () {
            ...
            if (
                ...
                this.isRenewable() &&
                this.isPaymentMethodIAP() &&
                !this.isLastInAppPurchasesFailure()
            ) {
                expiryType = 'payment_due';
            } else if (this.hasSubscriptionExpired()) {
                expiryType = 'expired';
            } else if (
                this.subscription.status === 'FREE_TRIAL_ACTIVE' &&
                this.subscription.auto_bill === true
            ) {
                expiryType = 'subscription_starts';
            } else if (this.subscription.auto_bill === true) {
                expiryType = 'renews';
            } else if (
                this.subscription.plan_type === 'business' ||
                this.subscription.status === 'FREE_TRIAL_ACTIVE' ||
                !this.subscription.auto_bill
            ) {
                expiryType = 'expiry';
            }
            return expiryType;

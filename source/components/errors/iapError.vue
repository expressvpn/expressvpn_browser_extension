...
<template>
    <div>
        <error-icon
            ...
            v-if="
                ![
                    'FREE_TRIAL_IAP_NON_RENEWAL_EXPIRED',
                    'SUBSCRIPTION_EXPIRED_NOGRACE_AUTOBILL_OFF',
                ].includes(IAPstate)
            "
        />
        ...
        <div
            v-if="
                [
                    'FREE_TRIAL_IAP_RENEWAL_FAILED',
                    'SUBSCRIPTION_EXPIRED_NOGRACE_AUTOBILL_ON',
                    'SUBSCRIPTION_ACTIVE_LAST_IAP_FAILED',
                ].includes(IAPstate)
            "
        >
        ...
        <div
            v-else-if="IAPstate == 'SUBSCRIPTION_EXPIRED_NOGRACE_AUTOBILL_OFF'"
        >
            <h1>{{ localize(`error_${IAPstate}_title`) }}</h1>
        ...
        <div class="button-container">
            <button class="primary" @click="handleMainAction">
                {{ localize(`error_${IAPstate}_main_button_label`) }}
            </button>
            <button class="secondary" @click="handleSubAction">
                {{ localize(`error_${IAPstate}_sub_button_label`) }}
            </button>
        </div>
    </div>
</template>
        ...
        handleMainAction() {
            switch (this.IAPstate) {
                ...
                case 'SUBSCRIPTION_ACTIVE_LAST_IAP':
                    this.createTab({
                        url: `${this.currentInfo.website_url}/support/troubleshooting/ios-in-app-purchases/#payment-failed`,
                    });
                    break;
                ...
                case 'SUBSCRIPTION_EXPIRED_NOGRACE_AUTOBILL_OFF':
                    this.createTab({
                        url: `${this.currentInfo.website_url}/order?source=free-trial&payment_method=ios-iap`,
                    });
                    break;
                default:
                    break;

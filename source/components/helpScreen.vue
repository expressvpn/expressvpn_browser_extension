        ...
        <div class="help-container">
            <div class="help-section">
                <div class="help-section-header">
                    {{ localize('menu_section_support_title') }}
                </div>
            ...
            <div class="help-section">
                <div class="help-section-header">
                    {{ localize('menu_section_about_title') }}
                </div>
                ...
                <div class="version-info" v-if="hasCookie" v-html="addAnchor('menu_item_extension_loggedin_text', '/subscriptions')" @click="checkForLinks"></div>
            </div>
        </div>
    </div>
</template>
...
export default {
    ...
    computed: {
        extensionVersion() {
            return this.localize('menu_item_extension_version_text').replace(
                '%VERSION%',
        ...
        appVersion() {
            let osName =
                this.currentInfo.os.charAt(0) +
                this.currentInfo.os.substr(1).toLowerCase();
            return this.localize('menu_item_app_version_text')
                .replace('%OS%', osName)
                .replace('%VERSION%', this.currentInfo.app.version);
        },
    },
    methods: {
        ...
        contactSupport() {
            this.createTab({
                url: `${this.currentInfo.website_url}/support/?utm_source=browser_extension&utm_medium=apps&utm_campaign=contact_support&utm_content=menu_contact_support`,
            });
        },
        talkToUs() {
            this.createTab({
                url: `${this.currentInfo.website_url}/support/?utm_source=browser_extension&utm_medium=apps&utm_campaign=contact_support&utm_content=menu_contact_support#open-chat`,
            });
        },
        suggestImprovements() {
    ...
    async mounted() {
        this.hasCookie = !!(await chrome.cookies.get({ url: 'https://www.expressvpn.com', name: "xv_ue" }));
    },
    components: {
        secondaryHeader,
        menuItem,

    ...
    <div>
        ...
        <div class="content">
            ...
            <div class="engine-state">
                {{
                    localize(`main_screen_status_vpn_${currentInfo.state}_text`)
                }}
            </div>
            ...
            <div class="locations-container">
                <div
                    :class="[
                        'current-location',
                        'location-box',
                        `location-box-${currentInfo.state}`,
                    ]"
                ...
                >
                    <img
                        ...
                        :src="flagStyle"
                        @error="this.setPlaceholder"
                    />
                    <div class="current-location-text">
                        <div class="location-box-text-type">
                    ...
                    <div v-if="['ready', 'connected'].includes(currentInfo.state)">
                        <img
                            v-svg-inline
                            class="current-location-more-icon"
                ...
                <transition mode="out-in">
                    <hint
                        v-if="!utils.isNullOrEmpty(hintText)"
                        :stringKey="hintText"
                        :iconName="hintIcon"
                        :type="hintType"
                    />
                    ...
                    <div
                        v-else-if="
                            ['ready', 'connected'].includes(currentInfo.state)
                        "
                        class="location-buttons"
                    >
                        <div
                            class="location-box"
                            v-for="location in visibleLocations"
                            v-bind:key="location.id"
                            @click="connectToLocation(location)"
                        >
                            <div class="location-box-text-type">
                                {{
                                    localize(
                                        `main_screen_${location.type}_location_text`
                                    )
                                }}
                            </div>
                            <div class="location-box-text-name">
            ...
            >
                <ratingPrompt
                    ...
                    :discardPrompt="onDiscardRatingPrompt"
...
export default {
        ...
        showRatingPrompt: function (newVal, oldVal) {
            if (newVal === true && oldVal === false) {
                chrome.storage.sync.get('rating', (storage) => {
                    if (
                        typeof storage.rating === 'object' &&
                        Object.keys(storage.rating).length > 0
                    ) {
    ...
    computed: {
        showRatingPrompt() {
            const requiredConnectionTime =
                __IS_ALPHA__ || process.env.NODE_ENV === 'development'
                    ? 0
                    : 15 * 60;
            ...
            const lastDiscardDate =
                parseInt(localStorage.getItem('lastDiscardDate'), 10) ||
                rating.lastDiscardDate;
            const lastFailedRateDate =
                parseInt(localStorage.getItem('lastFailedRateDate'), 10) ||
                rating.lastFailedRateDate;
        ...
        currentLocationTypeText() {
            let stringKey = `main_screen_${
                this.selectedLocation.is_smart_location ? 'smart' : 'selected'
            }_location_text`;
        ...
        flagStyle() {
            let filename =
                (this.selectedLocation.country_code
                    ? this.selectedLocation.country_code.toUpperCase()
                    : 'XV') + '.svg';
            return chrome.runtime.getURL('/images/flags/' + filename);
        },
        visibleLocations() {
            let locationList = [];
            if (
                this.currentInfo.smartLocation.id !== this.selectedLocation.id
            ) {
                locationList.push(
                    Object.assign({}, this.currentInfo.smartLocation, {
                        type: 'smart',
                    })
                );
            }

            for (let location of this.locationPicker.getRecentLocations(
                this.currentInfo.allLocationsList,
                this.currentInfo.recent_locations_ids
            )) {
                if (
                    location &&
                    location.id !== this.selectedLocation.id &&
                    location.id !== this.currentInfo.smartLocation.id
                ) {
                    locationList.push(
                        Object.assign({}, location, { type: 'recent' })
                    );
                }
            }
        ...
        hintText() {
            ...
            if (
                ...
                this.utils.getTimeDelta(
                    connectingTimes[nConnections - 1].startTime
                ) > 10 && // it’s been more than 10 seconds
                this.utils.getTimeDelta(
                    connectingTimes[nConnections - 1].startTime
                ) >
                    this.getAverageTopConnectionTime(2) * 1.25 &&
                this.discardHint === false // If the user already discarded the hint
            ) {
                text = 'hint_connection_delay_text';
                this.hintIcon = 'timer';
            } else if (
                ['ready', 'reconnecting', 'connecting'].includes(
                    this.currentInfo.state
                ) &&
                this.currentInfo.networkStatus !== 'has internet'
            ) {
                // GIVEN the user is NOT connected to the VPN and their internet connection is not clear
                ...
                let networkLockStatus = this.currentInfo.preferences
                    .traffic_guard_level
                    ? 'on'
                    : 'off';
    ...
    methods: {
        ...
        getAverageTopConnectionTime: function (nLastConnections) {
            let sortedConnectingTimes = this.currentInfo.connectingTimes.sort(
                (a, b) => b.delta - a.delta
            );
            let topNConnections = sortedConnectingTimes.slice(
                0,
                nLastConnections
            );

            return (
                topNConnections.reduce((total, el) => total.delta + el.delta) /
                nLastConnections
            );
        },
        ...
        setPlaceholder() {
            ...
            if (iconEl) {
                iconEl.src = chrome.runtime.getURL('/images/flags/XV.svg');
            }
        },
    },
    created() {
    ...
    .locations-container {
        ...
        .location-buttons {
            ...
            .location-box {
                ...
                &-text-name {
                    // do check for -webkit-line-clamp in the future
                    max-height: 40px;
                    margin: 0;
                    height: 30px;
                    display: flex;

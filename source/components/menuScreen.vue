...
<template>
    <div>
        <secondary-header
            stringkey="menu_header"
            :onBackClick="backToMainScreen"
        />
...
export default {
    ...
    methods: {
        canGetFreeTrial() {
            return (
                this.utils.isRegularUser(this.currentInfo.subscription) &&
                !this.utils.isPaymentMethodIAP(this.currentInfo.subscription)
            );
        },
        canRateExtension() {
            return (
                (['Chrome', 'Firefox'].includes(this.browserInfo.name) &&
                    ...
                    this.currentInfo.ratingData.everClickedMaxRating ===
                        false &&
                    !__IS_BETA__) ||
                __IS_ALPHA__ ||
        ...
        sendBetaFeedback() {
            ...
            const url = `https://expressv.typeform.com/to/Jla85o?browser_name=${

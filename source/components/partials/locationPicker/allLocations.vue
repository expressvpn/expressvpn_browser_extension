            ...
            <region-item
                ...
                @click.native="
                    currentExpandedRegion =
                        currentExpandedRegion !== region ? region : ''
                "
            />
            <div
                class="regionList"
                :data-region="region"
                :key="`list-${region}`"
                v-show="currentExpandedRegion === region"
            >
                <location-item
                    v-for="(
                        locations, country
                    ) in locationPicker.filterLocationsByProtocol(
                        locationsList,
                        currentInfo.preferences.preferred_protocol
                    )"
                    :key="`all-${country}`"
                    :model="locations[0]"
                    category="all"

...
<template>
    <div class="search-input-container">
        <input
            ...
            :placeholder="localize('location_picker_search_placeholder_text')"
            class="search-input"
            :style="isTyping ? 'padding-right: 35px;' : ''"
            autofocus
        />
    ...
    mounted() {
        if (process.env.NODE_ENV === 'development') {
            ...
            document
                ...
                .insertAdjacentHTML(
                    'afterend',
                    '<button id="mockSearch"></button>'
                );
            document
                .getElementById('mockSearch')
                .addEventListener('click', () => {
                    self.searchText = 'qqqq'; // make sure this text is not to be found in location names
                    self.updateInput();
                });
        }
    },

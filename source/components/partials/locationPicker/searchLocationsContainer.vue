...
export default {
    computed: {
        ...
        searchMatchedLocations: function () {
            ...
            let regionObj = [];
            let rxSearch = new RegExp(
                this.utils.escapeRegExp(
                    cleanText(this.searchText).replace(/ +(?= )/g, '')
                ),
                'i'
            );
            ...
            if (this.$parent.showCountryScreen === true) {
                Object.keys(this.currentInfo.allLocationsList).forEach(
                    (regionName) => {
                        let countryObj =
                            this.locationPicker.filterLocationsByProtocol(
                                this.currentInfo.allLocationsList[regionName],
                                this.currentInfo.preferences.preferred_protocol
                            )[this.$parent.currentCountry.name];
                        if (countryObj) {
                            countryObj.forEach(function (locationObj) {
                                if (
                                    rxSearch.test(cleanText(locationObj.name))
                                ) {
                ...
                for (let region in this.currentInfo.allLocationsList) {
                    regionObj = this.locationPicker.filterLocationsByProtocol(
                        this.currentInfo.allLocationsList[region],
                        this.currentInfo.preferences.preferred_protocol
                    );
                    for (let country in regionObj) {
                        // Allow search by partial country name and by country code
                        if (
                            rxSearch.test(cleanText(country)) ||
                            regionObj[country].some(
                                (el) =>
                                    el.country_code ===
                            ...
                            if (regionObj[country].length > 1) {
                                if (this.currentCountry) {
                                    regionObj[country].forEach(
                                        function (locationObj) {
                                            if (
                                                rxSearch.test(
                                                    cleanText(locationObj.name)
                                                )
                                            ) {
                                                filteredLocations.push({
                                                    isCountry: false,
                                    ...
                                    filteredLocations.push({
                                        isCountry: true,
                                        locationObj: regionObj[country][0],
                                    });
                                }
                            } else {
            ...
            return filteredLocations.sort(
                (loc1, loc2) =>
                    loc1.locationObj.sort_order - loc2.locationObj.sort_order
            );
        },
    },
    components: {

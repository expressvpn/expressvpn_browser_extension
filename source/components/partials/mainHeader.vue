...
<template>
    ...
    >
        <div class="menu-holder" @click="showMenu()">
            <img
                v-svg-inline
                class="menu-holder-icon"
                src="/images/icons/menu.svg"
            />
        </div>
        <div v-if="currentNotification" class="promobar-content">
            <div class="promobar-content-text">
                {{ notificationHeaderText }}
            </div>
            <div
                :class="[
                    'promobar-content-button',
                    `promobar-content-button-${buttonType}`,
                ]"
                @click="onPromoBarClick"
            >
...
export default {
    ...
    computed: {
        buttonType() {
            return ['splittunneling', 'update_available'].includes(
        ...
        currentNotification() {
            ...
            if (
                this.errorStates.includes(this.currentInfo.state) ||
                this.canShowPromobar !== true
            ) {
                return;
            }
            ...
            const standardizedVersion = this.currentInfo.app.latest_version
                ? this.currentInfo.app.latest_version.match(
                      /(?:\d+\.)(?:\d+\.)(?:\d+)/g
                  )[0]
                : '1.0.0';
            if (
                subscriptionObj.status === 'ACTIVE' &&
                this.utils.isLastInAppPurchasesFailure(subscriptionObj) &&
                this.utils.getTimeDelta(subscriptionObj.expiration_time).days >
                    -10 &&
                this.utils.getTimeDelta(subscriptionObj.expiration_time).days <=
                    1
            ) {
                ...
                subscriptionObj.status === 'FREE_TRIAL_ACTIVE' &&
                !this.utils.isInAppPurchasesRenewable(subscriptionObj)
            ) {
                notification = 'free_trial_expiring_soon';
            } else if (
                this.currentInfo.app.version &&
                ...
                let splittunnel =
                    this.currentInfo.preferences.split_tunnel || {};
                ...
                switch (mode) {
                    case 'vpn': // do not allow selected apps to use the VPN
                        showSplitTunnelNotification = noVpn.some(
                            function (splitApp) {
                                return (
                                    self.browserDict[splitApp.name] ===
                                    self.browserInfo.name
                                );
                            }
                        );
                        break;
                    case 'internet': // Only allow selected apps to use the VPN
                        showSplitTunnelNotification = !useVpn.some(
                            function (splitApp) {
                                return (
                                    self.browserDict[splitApp.name] ===
                                    self.browserInfo.name
        ...
        notificationBodyText() {
            ...
            if (this.currentNotification === 'subscription_expiring_soon') {
                locKey =
                    'promobar_subscription_expiring_soon_renewable_true_text';
            } else if (this.currentNotification === 'subscription_expired') {
                locKey =
                    'promo_bar_subscription_expiring_soon_renew_now_button_label';
            }
            ...
            if (this.currentNotification) {
                if (!this.currentNotification.includes('_expiring_soon')) {
                    return this.localize(
                        `promobar_${this.currentNotification}_header`
                    );
                }
                const remainingTime =
                    this.getTimeToExpiry().rawDifference / 1000;
                if (remainingTime <= 0) {
                    text = this.localize(
                        `promobar_${this.currentNotification}_expired_header`
                    );
                } else if (remainingTime < 60 * 60) {
                    text = this.localize(
                        `promobar_${this.currentNotification}_less_than_one_hour_header`
                    );
                } else if (remainingTime < 24 * 60 * 60) {
                    text = this.localize(
                        `promobar_${this.currentNotification}_hours_header`
                    ).replace(
                        '[hours]',
                        `${Math.ceil(remainingTime / (60 * 60))}`
                    );
                } else {
                    text = this.localize(
                        `promobar_${this.currentNotification}_days_header`
                    ).replace(
                        '[days]',
                        `${Math.ceil(remainingTime / (24 * 60 * 60))}`
                    );
                }
            }
            return text;
    ...
    methods: {
        ...
        onPromoBarClick: function (ev) {
            ...
            switch (this.currentNotification) {
                case 'subscription_expiring_soon':
                    if (
                        this.utils.isPaymentMethodIAP(
                            this.currentInfo.subscription
                        )
                    ) {
                        ...
                        if (this.isTimeElapsed()) {
                            url +=
                                '?utm_campaign=renew_subscription&utm_content=promobar_expired&utm_medium=apps&utm_source=browser_extension';
                        } else {
                            url +=
                                '?utm_campaign=renew_subscription&utm_content=promobar_renew_now_now&utm_medium=apps&utm_source=browser_extension';
                        }
                    }
                    break;
                ...
                case 'free_trial_expiring_soon':
                    url =
                        ...
                        '/order?source=free-trial&utm_campaign=free_trial&utm_content=promobar_free_trial_upgrade_now&utm_medium=apps&utm_source=browser_extension';
                    if (
                        this.utils.isPaymentMethodIAP(
                            this.currentInfo.subscription
                        )
        ...
        isTimeElapsed() {
            return (
                this.currentInfo.subscription.expiration_time -
                    Date.now() / 1000 <=
                0
            );
        },

...
<template>
    <div
        class="menu-item"
        @click="menuClick"
        v-if="isVisible && !model.isDivider"
    >
        <img
            v-svg-inline
            class="icon"
            :src="`/images/icons/${model.icon}.svg`"
            width="24"
            height="24"
            viewbox="0 0 24 24"
        />
...
export default {
    ...
    computed: {
        isVisible() {
            return typeof this.model.isVisible !== 'undefined'
            ...
            switch (this.model.id) {
                ...
                case 'other_devices':
                    url =
                        ...
                        '/setup?utm_source=browser_extension&utm_medium=apps&utm_campaign=browser_extension_links&utm_content=install_vpn_apps';
                    break;
                case 'get_free':
                    url =
                        this.currentInfo.subscription.referral_url +
                        '&utm_source=browser_extension&utm_medium=apps&utm_campaign=refer_friends&utm_content=bob_hamburger_referfriends';
                    break;
                default:
                    break;
            }

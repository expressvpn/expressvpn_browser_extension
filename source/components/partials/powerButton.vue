<template>
    <div class="power-button-container">
        <div
            :class="['power-button', `power-button-${currentInfo.state}`]"
            ...
            switch (this.currentInfo.state) {
                case 'ready':
                    chrome.runtime.sendMessage({
                        connectToSelectedLocation: true,
                    });
                    break;
                case 'connecting':
                case 'reconnecting':
                    if (
                        (parseInt(this.currentInfo.progress, 10) || 0) < 75 ||
...
.power-button {
    ...
    &-connecting,
    &-disconnecting {
        // v-svg-inline makes its props non reactive
        .power-button-icon path {

...
<template>
    <div class="popup-container">
        <div class="popup">
            ...
            <div v-show="ratingState === 'start'">
                <!-- using v-show because v-svg-inline . Should be removed in the future-->
                <img
                    v-svg-inline
                    class="logo"
                    src="/images/logo.svg"
                ...
                <div class="rating">
                    <div
                        ...
                        v-for="value in 5"
                        :key="value"
                        @click="handleRating(6 - value)"
                    />
                </div>
            </div>
            <div v-show="ratingState === '3star'">
                <span class="popup-header">{{
                    localize('rating_improve_header')
                }}</span>
                <div class="button-container">
                    <button class="primary" @click="improve('report')">
                        {{ localize('rating_improve_report_button') }}
                    </button>
                    <button class="secondary" @click="improve('suggest')">
                        {{ localize('rating_improve_suggest_button') }}
                    </button>
                </div>
            </div>
            <div v-show="ratingState === '5star'">
                <span class="popup-header">{{
                    localize('rating_thanks_text')
                }}</span>
                <div class="button-container">
                    <button class="primary" @click="openWebStore">
                        {{ localize('rating_prompt_ok_button') }}
                    </button>
                    <button class="secondary" @click="discardPrompt">
                        {{ localize('rating_no_thanks_text') }}
                    </button>
                </div>
            </div>
        </div>
    ...
    methods: {
        openWebStore() {
            ...
            if (currentBrowser === 'Chrome') {
                url =
                    'https://chrome.google.com/webstore/detail/expressvpn-for-chrome/fgddmllnllkalaagkghckoinaemmogpe/reviews';
        ...
        improve(type) {
            ...
            switch (type) {
                ...
                case 'suggest':
                    ...
                    url = `https://expressv.typeform.com/to/Jla85o?browser_name=${
                        systemInfo.browser.name
                    }&browser_ver=${systemInfo.browser.version}&ext_ver=${
                        chrome.runtime.getManifest().version
                    }&os_name=${systemInfo.os.name}&os_ver=${
                        systemInfo.os.version
                    }&app_ver=${this.currentInfo.app.version}&locale=${
                        this.currentInfo.locale
                    }&is_beta=false`;
                    break;
                default:
                    break;

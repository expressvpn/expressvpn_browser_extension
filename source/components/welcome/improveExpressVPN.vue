...
<template lang="html">
    <div class="improveExpressVPN">
        ...
        <p>
            {{ localize('welcome_help_improve_expressvpn_p1_text') }}
            <a
                v-if="browserInfo.name === 'Firefox'"
                href="#"
                @click="
                    createTab({
                        url: 'https://addons.mozilla.org/en-US/firefox/addon/expressvpn/privacy/',
                    })
                "
                >{{
                    localize('welcome_help_improve_expressvpn_learn_more_text')
                }}</a
            >
        </p>

        <div class="button-container">
            <button class="primary" @click="helpImprove(true)">
                {{
                    localize('welcome_help_improve_expressvpn_ok_button_label')
                }}
            </button>
            <button class="secondary" @click="helpImprove(false)">
                {{
                    localize('welcome_help_improve_expressvpn_no_button_label')
                }}
            </button>
        </div>
    </div>
...
export default {
    ...
    async mounted() {
        await chromeStorage.set('hasSeenHelpImproveScreen', true);
    },
};
</script>

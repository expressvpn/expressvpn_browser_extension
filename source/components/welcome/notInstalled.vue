...
<template lang="html">
    <div class="scenes">
        <img
            v-svg-inline
            class="logo"
            src="/images/logo/expressvpn-logo-red.svg"
            width="167"
            height="32"
        />
        <div class="scene" v-if="scene <= 3">
            <div class="scene-text">
                {{ localize(`welcome_not_installed_scene_${scene}_text`) }}
            </div>
            <img
                class="scene-image"
                :src="`/images/illustrations/welcome_not_activated_${scene}.png`"
            />
        </div>
        <div class="scene" v-else>
            <div class="scene-header">
                {{ localize('welcome_not_installed_buy_header') }}
            </div>
            <div class="scene-header-secondary">
                {{ localize('welcome_not_installed_buy_text') }}
            </div>
            <div class="animation-container list">
                <ul class="check-list">
                    <li v-for="index in 4" :key="index">
                        {{
                            localize(
                                `welcome_not_installed_reasons_li_${index}_text`
                            )
                        }}
                    </li>
                </ul>
        ...
        <div class="button-container">
            ...
            <button
                ...
                @click="
                    createTab({
                        url: `${currentInfo.website_url}/order?utm_content=welcome_screen_not_installed&utm_medium=apps&utm_source=browser_extension`,
                    })
                "
            >
                {{ localize('welcome_not_installed_get_button_label') }}
            </button>
            <button
                class="secondary"
                @click="$parent.$emit('showImproveExpressVPN')"
            >
                {{ localize('welcome_not_installed_existing_button_label') }}
            </button>
        </div>
    </div>
</template>

  ...
  methods: {
    ...
    isInGracePeriod() {
      return (
        ...
        ['ACTIVE', 'FREE_TRIAL_ACTIVE', 'MULTI_DEVICE_FREE_TRIAL_ACTIVE'].includes(this.subscription.status)
      );
    },
    isRegularUser() {
    ...
    isLastInAppPurchasesFailure() {
      ...
      return (
        isIAP &&
        this.subscription.payment_method.toLowerCase() === 'app_store_auto_renewable' &&
        this.subscription.last_auto_bill_failure === true
      );
    },
    getLastIAPFatalError() {
      ...
      if (this.subscription.status === 'FREE_TRIAL_EXPIRED' && this.isLastInAppPurchasesFailure() && this.subscription.auto_bill === true) {
        ...
        this.subscription.status === 'FREE_TRIAL_EXPIRED' &&
        this.hasSubscriptionExpired() === true &&
        this.subscription.auto_bill === false
      ) {
  ...
  computed: {
    ...
    state() {
      ...
      if (this.getTimeToExpiry().rawDifference < 0) {
        ...
        this.getTimeToExpiry().days < this.DAYS_TO_EXPIRING_STATE &&
        !this.isRenewable() &&

...
function initializeVueApp() {
  const app = new Vue({
    ...
    mounted() {
      ...
      chrome.storage.local.get(null, (storage) => {
        ...
        setInterval(
          (function message() {
            if (typeof storage.messages === 'object') {
    ...
    computed: mapGetters(['currentView']),
  });
  ...
  chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
    if (message.state) {
      ...
      if (
        utils.getLastIAPFatalError(currentInfo.subscription) !== null &&
        !app.errorStates.filter((state) => state !== 'subscription_expired').includes(currentInfo.state)
      ) {
      ...
      if (app.errorStates.indexOf(currentInfo.state) === -1) {
        if (
          (currentInfo.state === 'connecting' || currentInfo.state === 'reconnecting') &&
          app.currentView !== 'SettingsGeneral' &&
          app.currentView !== 'mainScreen'
        ) {
      ...
      app.$store.dispatch('setlangList', [
        ...
        {
          code: 'pt_BR',
          label: app.localize('language_dropdown_portuguese'),
        },
  ...
  chrome.storage.session.get('localizedStrings', (storage) => {
      navigator.localizedStrings  = storage.localizedStrings;
  })
}

// Migrate settings to chrome.storage
if (!localStorage.getItem('localstorage2chrome')) {
  let storageObj = {};
  let hasData = false;
  ...
  if (hasData) {
    ...
    chrome.storage.local.set(storageObj, () => {
      localStorage.setItem('localstorage2chrome', chrome.runtime.getManifest().version);
